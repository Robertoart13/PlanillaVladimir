...
const QUERIES = {
   ...
   QUERIES_SELECT: `
               SELECT * FROM gestor_empleado_tbl
                where  id_empresa = ? AND estado_empleado_gestor = 1
      `,
};
...
const obtenerTodosDatos = async (id_empresa, database) => {
   try {
      return await realizarConsulta(QUERIES.QUERIES_SELECT, [id_empresa], database);
   } catch (error) {
      return manejarError(
         error,
         500,
 ...
 * ====================================================================================================================================
 */
const Gestor_Planilla_Empleados_Options = { 
      empleadosPlanillaOptions: obtenerListaCompleta, // Método que obtiene la lista completa, con validaciones y permisos.
};

export default Gestor_Planilla_Empleados_Options;
