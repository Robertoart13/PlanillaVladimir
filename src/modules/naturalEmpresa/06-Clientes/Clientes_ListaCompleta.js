...
const QUERIES = {
   ...
   QUERIES_SELECT: `
         SELECT 
            u.*, 
            COALESCE(e.nombre_comercial_empresa, 'No aplica') AS nombre_empresa
         FROM 
            usuarios_tbl AS u
         LEFT JOIN 
            empresas_tbl AS e 
            ON u.id_empresa_usuario = e.id_empresa 
         WHERE 
            u.rol_usuario IN ('usuario', 'supervisor');
