...
const QUERIES = {
   ...
   QUERIES_SELECT: `
         SELECT
            e.*,
            u.nombre_usuario,
            COALESCE(CONCAT(emp.nombre_empleado, ' ', emp.apellidos_empleado), 'N/A') AS nombre_completo_empleado
         FROM
            eventos_tbl e
         INNER JOIN
            usuarios_tbl u ON e.id_usuario_evento = u.id_usuario
         LEFT JOIN
            empleados_tbl emp ON e.id_empleado_evento = emp.id_empleado
         WHERE
