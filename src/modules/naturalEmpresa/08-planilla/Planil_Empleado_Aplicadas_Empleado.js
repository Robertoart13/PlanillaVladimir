...
SELECT
  ...
  plan.*,
  usr.*,
  iban.numero_cuenta_iban,
  pst.nombre_puesto
FROM
  empleado_planilla_detalle_tbl epd
INNER JOIN empleados_tbl emp ON epd.id_empleado_epd = emp.id_empleado
INNER JOIN empresas_tbl empz ON epd.id_empresa_epd = empz.id_empresa
INNER JOIN planilla_tbl plan ON epd.planilla_id_epd = plan.planilla_id
INNER JOIN usuarios_tbl usr ON epd.id_usuario_creador_epd = usr.id_usuario
LEFT JOIN cuentas_iban_tbl iban ON emp.id_empleado = iban.id_empleado
LEFT JOIN puestos_tbl pst ON emp.id_puesto = pst.id_puesto
WHERE
  epd.planilla_id_epd = ?
  AND epd.marca_epd = 1;
 ...
 * ====================================================================================================================================
 */
const obtenerTodosDatos = async (planilla, database) => {

   try {
      // Ejecuta la consulta SQL para obtener los datos de la tabla
      return await realizarConsulta(QUERIES.QUERIES_SELECT, [planilla], database);
   } catch (error) {
      return manejarError(
         error,
         500,
...
const obtenerListaCompleta = async (req, res) => {
   try {
      ...
      const resultado = await obtenerTodosDatos(res?.transaccion?.data.id_planilla.planilla_id, res?.database);
 ...
 * ====================================================================================================================================
 */
const Planil_Empleado_Aplicadas_Empleado = {
   ListaPlanilla: obtenerListaCompleta, // Método que obtiene la lista completa, con validaciones y permisos.
};

export default Planil_Empleado_Aplicadas_Empleado;  
