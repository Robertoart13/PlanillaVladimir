   ...
   return {
      ...
      ajax: {
         ...
         dataSrc: (response) => {
            ...
            let dataArray = Array.isArray(response.array) ? response.array : [];

            // Filtrar por empresa_id - maneja casos donde empresa_id puede ser undefined
            dataArray = dataArray.filter((item) => {
               // Si el item no tiene empresa_id, no lo incluimos
               if (!item.empresa_id) {
                  return false;
               }
               // Solo incluir items que pertenezcan a la empresa del usuario
               return item.empresa_id === 13;
            });
